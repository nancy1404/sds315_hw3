...
# b) The richer the area, the higher the gas prices 
# since asking for area, divide into zipcode.

library(dplyr)
library(boot)

# group by Zipcode (area) w/ price and income average
...
# scatterplot (relationship btw gas prices and avg income)
ggplot(area_avg, aes(x= avg_income, y = avg_price)) +
  ...
  labs(
    ...
    caption = "* this scatterplot shows relationship between gas prices and average income levels by area.",
    x = "Average Income ($)",
    y = "Average Gas Price ($)"
  )

# bootstrap for difference in gas prices by income categories (richest vs. poorest areas)

# total of 17 zipcodes found or 17 areas found
richest_area <- area_avg |>
  arrange(desc(avg_income)) |>
  head(8.5) # half most richest areas

pooreast_area <- area_avg |>
  arrange(avg_income) |>
  head(8.5) # half most poorest areas

selected_areas <- c(richest_area$Zipcode, pooreast_area$Zipcode)
area_selected <- gasprices |>
  filter(Zipcode %in% selected_areas) |>
  mutate(
    IncomeCategory = ifelse(Zipcode %in% richest_area$Zipcode, "Richest", "Non-Richest")
  )

# Mean price for richest and poorest areas
mean_price_income <- mean(Price ~ IncomeCategory, data = area_selected)
mean_price_income

set.seed(111) # for reproducibility
# Bootstrap for price differences
boot_price_income <- do(10000) * diffmean(Price~IncomeCategory, data = resample(area_selected))
